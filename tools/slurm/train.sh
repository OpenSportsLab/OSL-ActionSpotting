...
#SBATCH --output=tools/slurm/logs/%x_%j.out # Path for standard output (%x is job name, %j is job ID)
#SBATCH --error=tools/slurm/logs/%x_%j.err  # Path for standard error
...
# configs/e2espot/e2espot.py --cfg-options model.multi_gpu=True work_dir=outputs/e2e/rny002_gsm_trois model.backbone.type=rny002_gsm
