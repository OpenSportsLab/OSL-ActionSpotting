...
def createjson(split, json_file):
    ...
    for game in getListGames(split):
        ...
        # for feature in sorted(glob.glob(f'{DIR}/{game}/*_ResNET_TF2_PCA512.npy')):
            feature = os.path.join(DIR,game,f'{half}_ResNET_TF2_PCA512.npy')
            # print(feature)
            label_file = os.path.join(DIR,game, "Labels-v2.json")
            ...
            if "1_" in os.path.basename(feature):
                ...
                half = 2
            video["path_video"] = os.path.join(game, f"{half}.mkv")
            video["path_features"] = os.path.join(game, os.path.basename(feature)) # feature #fold+".npy"
    ...
    JSON_all = f'path/to/SoccerNet/{json_file}'
    with open(JSON_all, 'w') as f:
        json.dump(data, f)

createjson("train", "train.json")
createjson("valid", "val.json")
